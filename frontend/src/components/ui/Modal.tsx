...
export function Modal({ open, onClose, title, children, footer, footerClassName, maxWidth = "max-w-lg" }: ModalProps) {
  return (
    <Dialog.Root open={open} onOpenChange={(nextOpen) => !nextOpen && onClose()}>
      <Dialog.Portal>
        ...
        <Dialog.Content
          className={cn(
            "fixed left-1/2 top-1/2 z-50 flex max-h-[min(90vh,calc(100dvh-1rem))] w-[calc(100%-1rem)] -translate-x-1/2 -translate-y-1/2 flex-col rounded-lg border border-border bg-card text-card-foreground shadow-xl sm:w-[calc(100%-2rem)]",
        ...
        >
          ...
          {footer && (
            <div className={cn("flex flex-col-reverse gap-2 border-t border-border px-5 py-4 sm:flex-row sm:justify-end", footerClassName)}>
              {footer}
            </div>
          )}
        </Dialog.Content>
